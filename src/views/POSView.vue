  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <header class="bg-primary-600 text-white shadow-lg">
      <div class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <router-link to="/" class="btn-secondary flex items-center gap-1">
    ...
    <div class="container mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card">
          <BarcodeScanner 
            @productFound="handleProductFound"
        ...
        <div class="card">
          <h3 class="text-lg font-semibold mb-4 flex items-center gap-2">
            <Search class="w-5 h-5" /> Búsqueda manual
          </h3>
          <div class="flex gap-2">
            ...
            />
            <button @click="searchProducts" class="btn-primary">Buscar</button>
          </div>

          <div v-if="searchResults.length > 0" class="mt-4 space-y-2">
            <h4 class="font-semibold text-gray-700">Resultados:</h4>
            <div v-for="product in searchResults" :key="product.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
              <div>
        ...
        <div class="card sticky top-6">
          <h3 class="text-lg font-semibold mb-4 flex items-center justify-between">
            <span class="flex items-center gap-2">
              <ShoppingBag class="w-5 h-5" /> Carrito
            </span>
          ...
          <div v-else class="space-y-3 max-h-96 overflow-y-auto">
            <div v-for="item in cartStore.items" :key="item.product.id" class="p-3 bg-gray-50 rounded-lg">
              <div class="flex justify-between items-start mb-2">
                <div class="flex-1">
                  <p class="font-medium text-sm">{{ item.product.name }}</p>
                  <p class="text-xs text-gray-600">{{ item.product.barcode }}</p>
                </div>
                <button @click="cartStore.removeItem(item.product.id)" class="text-red-600 hover:text-red-800">
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <button @click="cartStore.updateQuantity(item.product.id, item.quantity - 1)" class="w-7 h-7 bg-gray-200 rounded hover:bg-gray-300 flex items-center justify-center">
                    <Minus class="w-4 h-4" />
                  </button>
          ...
          <div v-if="cartStore.items.length > 0" class="mt-6 pt-4 border-t border-gray-200">
            ...
            <div class="space-y-2">
              <button @click="processSale" class="w-full btn-success py-3 text-lg flex items-center justify-center gap-2">
                <CheckCircle class="w-5 h-5" /> Procesar venta
              </button>
              <button @click="clearCart" class="w-full btn-danger py-3 text-lg flex items-center justify-center gap-2">
                <X class="w-5 h-5" /> Limpiar carrito
              </button>
            </div>
          </div>
        </div>
    ...
    <div v-if="showSaleModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click="showSaleModal = false">
      <div class="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-2xl" @click.stop>
        <div class="text-center">
          ...
          <button @click="showSaleModal = false" class="btn-primary w-full">Aceptar</button>
        </div>
      </div>
    </div>
  </div>
...
import { 
  ShoppingCart, ShoppingBag, Search, Plus, Minus, Trash2, CheckCircle, X, Home 
...
const processSale = () => {
  ...
  // Aquí enviar venta al backend
  console.log('Venta procesada:', { items: cartStore.items, total: cartStore.total, date: new Date().toISOString() })
  alert('Venta procesada: ' + { items: cartStore.items, total: cartStore.total, date: new Date().toISOString() })


  setTimeout(() => cartStore.clearCart(), 2000)
}

// Limpiar carrito
const clearCart = () => {
  if (confirm('¿Estás seguro de limpiar el carrito?')) cartStore.clearCart()
}
</script>
